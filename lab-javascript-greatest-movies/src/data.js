...
//   const sum = avgDrama.reduce((a,c) => a+c);
...
function turnHoursToMinutes(movieArr) {
  ...
  const duration = movies.filter(m => {
    //let time = m.duration.split(" ");
    const hours = m.duration.match(/\d/);
    console.log(hours);
    
    // return m.duration = "asdf";
  });
  return duration;
}
